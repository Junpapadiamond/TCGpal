...
|---|---|---:|---|---|
| WS-IDENTITY | Exact-print selection and listing fidelity | Large refactor | Single-print proof + catalog coverage fixed at `289d334`; D-OP-BASE-PROOF sampler built, never run live | Run `npm run sample:base-proof`, adjudicate the sheet, then score it |
...
7. D-OP-BASE-PROOF: whether a One Piece *multi-print* base print may be proven by the absence of any sibling marker. This is the remaining 9 of 10 recall misses. It trades a real guardrail for coverage and must not be written before the falsifiable test: take the `unknown` rows from three One Piece base cards (Nami OP01-016 has 13, Zoro OP06-118 24, Shanks OP09-001 25), open each listing, and count how many are genuinely the base print. Ship at >=90% precision, kill below 80% and improve the `inspect_first` copy instead. The instrument is built (`npm run sample:base-proof`) and has never been run against live eBay, so the sample does not exist yet. One correction it forces: those 13/24/25 are `printMatch` tallies, not the precision denominator. A row also excluded as graded, sealed, unknown-shipping, or language-conflicting would stay excluded after the rule changed, so it says nothing about whether the relaxed rule picks the right artwork; only rows whose *sole* exclusion is `identity_unverified` are adjudicated, and the sampler reports the rest separately with the code that also blocks them. Expect a denominator below 62. The exact-print price guard is deliberately counted as a separate blocker rather than assumed to clear. **This decision is coupled to D-DETAIL-BUDGET:** `condition_unstated` is an `exclude`, and at the default budget of 12 of 50 most rows are never enriched, so a large share of print-blocked rows leave the denominator for a condition reason rather than a print one. A small adjudicable set is therefore partly a budget artifact and is not by itself evidence the rule frees no supply — the sampler detects when that cause dominates and says so in both the console and the sheet. Re-run against a dev server started with `EBAY_DETAIL_BUDGET=50` to separate the two before reading the result.
<!-- progress:end -->
