...
Current operational state: the founder requested Chrome cookie/session reuse; the existing Chrome session successfully opened Vercel project settings without exporting any cookie. The production WHATNOT_APIFY_TOKEN exists, but no Mercari token, linked shared variables or connected Redis store is present. The Upstash Redis installer is prepared at Accept and Create; the [Marketplace offers Free](https://vercel.com/marketplace/upstash/upstash-kv), but creating the integration accepts Vercel/Upstash terms and shares account ID, email or usage data. Specific approval for that step and free-only provisioning is pending. No integration, production token or flag has been changed.
