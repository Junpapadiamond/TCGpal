...
- Redis accepts a complete `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN` pair, or Vercel Marketplace's `KV_REST_API_URL` / `KV_REST_API_TOKEN` pair. Explicit Upstash configuration takes precedence; credentials from different pairs are never mixed and read-only tokens cannot back the spending counter. When neither complete pair is available or Redis fails, ordinary rate limiting and caching fall back to bounded in-process memory. Paid Apify calls instead fail closed: their lifetime 20-start counter requires durable Redis and is never reset on deployment.
