...
const en = {
  ...
  header: {
    ...
    saveCard: "Save card",
    savedCard: "Saved",
    savedEmpty: "Saved cards stay on this browser.",
  ...
  result: {
    ...
    yes: "Yes",
    notYet: "Not yet",
    marketPrice: "TCGplayer market",
    listedRange: "listed range",
    liveAndScreened: (live: number, screened: number) => `${live} live · ${screened} screened out`,
    sourcesAt: (sources: string, time: string) => `${sources} · ${time}`,
    sourcesLive: (live: number, missing: number) => `${live} sources live · ${missing} not connected`,
    candidateCount: (n: number) => `${n} candidate${n === 1 ? "" : "s"}`,
    lensNote: "Risk labels are rule-based — not grade predictions",
    trackCard: "Track this card",
    trackingCard: "Tracking this card",
    trackBody: (price: string) => `Keep an eye on listings that beat ${price}. Saved in this browser.`,
...
const zh: Dict = {
  ...
  result: {
    ...
    yes: "是",
    notYet: "还没",
    marketPrice: "TCGplayer 市价",
    listedRange: "在售区间",
    liveAndScreened: (live: number, screened: number) => `${live} 条在售 · 筛掉 ${screened} 条`,
    sourcesAt: (sources: string, time: string) => `${sources} · ${time}`,
    sourcesLive: (live: number, missing: number) => `${live} 个实时来源 · ${missing} 个未接入`,
    candidateCount: (n: number) => `${n} 个候选`,
    lensNote: "风险标签由规则计算，不是评级预测",
    trackCard: "追踪这张卡",
    trackingCard: "正在追踪",
    trackBody: (price: string) => `关注低于 ${price} 的新商品；状态保存在这台浏览器。`,
