...
const en = {
  ...
  result: {
    ...
    inspectFirstBody: "This listing may be useful, but its text does not prove it is the artwork you confirmed. Compare the two images and verify the live page before deciding.",
    ...
    listingEvidencePhoto: "Listing evidence",
    confirmedReferenceAlt: (name: string, number: string, variant: string) => `Confirmed card reference: ${name} ${number}${variant ? ` ${variant}` : ""}`,
    listingEvidenceAlt: (title: string) => `Listing evidence photo: ${title}`,
...
const zh: Dict = {
  ...
  result: {
    ...
    inspectFirstBody: "这条商品可能有用，但文字信息还不能证明它就是你确认的卡图。请对照两张图片，并在决定前核对商品页面。",
    inspectListing: "核对商品",
    ...
    confirmedReference: "已确认参考图",
    listingEvidencePhoto: "商品证据图",
    confirmedReferenceAlt: (name: string, number: string, variant: string) => `已确认卡片参考图：${name} ${number}${variant ? ` ${variant}` : ""}`,
    listingEvidenceAlt: (title: string) => `商品证据照片：${title}`,
