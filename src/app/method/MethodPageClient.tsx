...
const copy = {
  ...
  zh: {
    eyebrow: "方法与证据契约",
    title: "TCGlens 如何比较商品",
    intro: "TCGlens 面向已经知道要买哪张卡的买家：先确认确切卡图，再比较未评级单卡的在售商品；证据不足时会明确不做推荐。",
    back: "开始比较卡片",
    sections: [
      ...
      ["市场参考价代表什么", "参考价是已确认版本在可比品相语境下的商品本身汇总价格，不含运费与税。TCGlens 只拿商品标价与这项参考价比较；运费和预估税会在结账成本明细中单独列出，不会被用来制造“高于市场”的警告。"],
      ...
      ["TCGlens 不做什么", "TCGlens 不根据照片评级、不提供鉴真或仿品检测、不预测价格、不声称未经验证的成交记录，也不会把卖家称为骗子。品相仍是卖家声明；缺少卖家资料会标为“信息不足”，而不是自动判为高风险。"],
      ["保存凭据与隐私", "稳定凭据是一份保留 30 天的不可变快照，不代表商品仍然在售。页面会显示保存时间并提供重新查询实时商品的按钮。TCGlens 只为服务器已验证的纯卡片搜索建立凭据，会移除买家邮编，也不会通过这条路径公开贴入或手动填写的商品资料。"],
    ...
    <main className="min-h-screen bg-[#f4f7f3] text-[#24312f]">
      <header className="border-b border-[#d6ded5] bg-[#fcfbf6]">
        <div className="mx-auto flex max-w-[1040px] items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          <Link href="/" aria-label="TCGlens home"><Image src="/tcgpal-logo-horizontal.svg" alt="TCGlens" width={142} height={41} preload /></Link>
