...
- eBay Browse is the verified production concrete-listing source. The founder authorized the bounded Apify price-display pilot and on September 23 approved one Mercari Console test (90 seconds/$0.05/three results), followed by activation on success. Activation uses `CROSS_MARKET_PRICE_PILOT_ENABLED`, provider credentials and the existing durable shared counter of 20 lifetime application starts, without resets. Each start is capped at three results and $0.03 for Whatnot / $0.05 for Mercari; current Actor prices must pass a preflight before reserving a start. Including the separate approved Console test, new spending is bounded by $1.05. Old credentials and `CROSS_MARKET_APIFY_ENABLED` must not activate it. Whatnot requires a checked money unit; Mercari uses the explicitly gated US proxy adapter. Authorization is not evidence of successful acquisition or platform permission. See `docs/cross-market-pilot.md` for actual rollout state. eBay Catalog may resolve confirmed-card identity/ePID but is never inventory. TCGCSV/TCGplayer and PriceCharting are references, never ranking candidates.
- Cross-market asking prices may display active raw USD listings with unknown shipping, buyer fees, condition or unresolved print evidence, clearly labeled for review. Known number/print/language conflicts, excluded products and sold/graded listings do not enter that display. Display is separate from deterministic complete-cost ranking; missing charges never become zero to produce a winner.
...
- Listed median is not sales-based market price: neither TCGCSV `midPrice` nor inline TCGplayer `mid` may replace a missing `marketPrice`/`market`. Missing mandatory buyer fees, like missing shipping, prevent a complete-cost winner. Otherwise eligible rows may show a deterministic, pre-tax break-even verdict for the missing charges; they never become lens winners on that basis.
...
- Founder-authorized capped Apify production pilot through the existing Whatnot and Mercari adapters only, with the limits and activation requirements above. No scheduled harvesting, marketplace-account credentials, unsupported complete-cost claims, or expansion beyond the prepared pilot quota.
