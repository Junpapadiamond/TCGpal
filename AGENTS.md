...
- eBay Browse is the verified production concrete-listing source. The founder explicitly requested production activation of the bounded Apify price-display pilot on September 14. Activation uses `CROSS_MARKET_PRICE_PILOT_ENABLED`, provider credentials, a durable shared counter of 20 lifetime starts, and a $0.03/three-result cap per start; old credentials and `CROSS_MARKET_APIFY_ENABLED` must not activate it. Whatnot requires a checked money unit; Mercari uses the explicitly gated US proxy adapter. This activation request is not evidence of successful acquisition or platform permission, and automatic approval separately requires an answer before the proposed additional 90-second Console test. See `docs/cross-market-pilot.md` for actual rollout state. eBay Catalog may resolve confirmed-card identity/ePID but is never inventory. TCGCSV/TCGplayer and PriceCharting are references, never ranking candidates.
