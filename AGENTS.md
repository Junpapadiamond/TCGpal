...
- eBay Browse is the default live concrete-listing source. Founder-requested Whatnot/Mercari Apify adapters are an opt-in pilot behind `CROSS_MARKET_APIFY_ENABLED`; current acceptance evidence and the public activation gate live in `docs/cross-market-pilot.md`. Label their access `third_party_provider`, never official or licensed. eBay Catalog may resolve confirmed-card identity/ePID but is never inventory. TCGCSV/TCGplayer and PriceCharting are references, never ranking candidates.
...
- Bounded founder-requested Whatnot (`epicscrapers/whatnot-scraper`) and Mercari (`getascraper/mercari-us-scraper`) Apify pilot adapters, only on an explicit user search and an operator-enabled rollout switch. Enforce per-run charge/result/time limits, a shared production daily budget, validated monetary/status fields, source attribution, and unknown checkout charges. No direct marketplace scraping, proxy rotation, cookies, or block bypass. Live field validation and source-rights/operational review remain required before public activation; see `docs/cross-market-pilot.md`.
