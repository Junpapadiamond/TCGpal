...
- eBay Browse is the only live concrete-listing source. The founder retired Apify acquisition; old credentials and rollout flags must not activate paid scraping. Self-built Whatnot/Mercari acquisition remains separate Frontier Research until the promotion gates pass; see `docs/cross-market-pilot.md`. eBay Catalog may resolve confirmed-card identity/ePID but is never inventory. TCGCSV/TCGplayer and PriceCharting are references, never ranking candidates.
