...
- eBay Browse is the only verified production concrete-listing source. The founder has reopened a bounded Apify price-display pilot; old credentials and `CROSS_MARKET_APIFY_ENABLED` must not activate it. New acquisition requires `CROSS_MARKET_PRICE_PILOT_ENABLED`, provider credentials, a durable shared spending counter, and completed rollout review. Mercari proxy use currently has one Console-test exception only, not recurring production authorization; see `docs/cross-market-pilot.md`. eBay Catalog may resolve confirmed-card identity/ePID but is never inventory. TCGCSV/TCGplayer and PriceCharting are references, never ranking candidates.
